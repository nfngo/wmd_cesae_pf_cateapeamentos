<div class="w-100 p-4 mt-5">
    <div class="row">
        <div class="col-12">
            <p class="my-4 text-uppercase fs-5">Custos Apeamentos</p>
            ...
            <table id="custosApeados" class="table">
                ...
                @foreach( $custos_apeados as $item)
                    <tr>
                        ...
                        <td>{{ number_format( $item-> total_real_venda - $item->custo_bald_apea,  1, ',', ' ') }}€</td>
                        <td>{{ number_format((($item-> total_real_venda - $item->custo_bald_apea)) * 100/ $item->total_real_venda,  1, ',', ' ')}}%</td>
                        <td>{{ number_format((($item-> total_plan_venda - $item->custo_bald_apea) * 100 / $item-> total_plan_venda), 1, ',', ' ' ) }}%</td>
